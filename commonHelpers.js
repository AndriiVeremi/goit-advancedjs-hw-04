...
</a>`).join("")}const s={form:document.querySelector("#search-form"),input:document.querySelector("input"),gallery:document.querySelector(".gallery"),loadBtn:document.querySelector(".js-load-btn"),logo:document.querySelector(".js-pixa")};s.form.addEventListener("submit",L);s.loadBtn.addEventListener("click",v);i(s.loadBtn,!1);i(s.logo,!0);function i(n,t){n.classList.toggle("hidden",!t)}const h=new b(".js-gallery a",{captionDelay:250,captionsData:"alt"});let u="",d=1;async function L(n){n.preventDefault(),f();const t=s.input.value.trim().toLowerCase();if(!t){c.show({title:"Ops!",message:"Enter something to search!",position:"topRight"});return}if(u===t){c.show({title:"Ops!",message:"Enter something!",position:"topRight"});return}u=t;try{const o=await p(u,d),a=o.data.totalHits,e=o.data.hits;if(a===0){c.show({title:"Ops!",message:"We're sorry, but you've reached the end of search results.",position:"topRight"});return}i(s.logo,!1),s.gallery.insertAdjacentHTML("afterbegin",g(e)),c.show({title:"Hey",message:`Hooray! We found ${a} images.`,position:"topRight"}),i(s.loadBtn,!0),h.refresh()}catch(o){console.log(o),f(),i(s.loadBtn,!1)}}async function v(n){n.preventDefault(),d+=1;try{const t=await p(u,d),o=t.data.hits,a=t.data.totalHits;if(!(d<Math.ceil(a/40))){c.show({message:"We're sorry, but you've reached the end of search results.",position:"topRight"}),i(s.loadBtn,!1);return}s.gallery.insertAdjacentHTML("beforeend",g(o)),S(),h.refresh()}catch(t){console.log(t),f()}}function f(){s.gallery.innerHTML="",u="",d=1}function S(){const{height:n}=document.querySelector(".gallery").firstElementChild.getBoundingClientRect();window.scrollBy({top:n*2,behavior:"smooth"})}
